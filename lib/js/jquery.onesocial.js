...
/*------------------------------------------------------------ actually finally inserting the buttons -----------------------------------------------------------------------*/
    ...
    widget.twitter.tweet = function(node, options)
    {
        ...
        var button = $('<a class="twitter-share-button" href="https://twitter.com/share" data-url="' + url + '">Tweet</a>');
...
/*------------------------------------------------------------ anonymous functions ---------------------------------------------------------------------------------------*/
    ...
    var getSupportedLanguage = function(want, api)
    {
        ...
        else
        {
            ...
            var shortwant = want.substring(0, 2);
            ...
            {
                $.each(languages[api], function(key, value){
                    /* This is a funky startsWith check */
                    if(key.slice(0, 2) == shortwant)
                    {
                        return key;
                    }
                });
