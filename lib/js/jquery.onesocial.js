    ...
    var defaults =
    {
        ...
        facebook: {
            ...
            like:
            {
                ...
                href: 'pageUrl', // url, 'pageUrl' for current page, 'rootUrl' for main page
                'kid-directed-site': 'false', // false / true
                layout: 'button_count', // standard / button_count / button / box_count
                ref: '', // default is None
                share: 'false', // true / false
                'show-faces': 'false' // true / false
                //width: '' // use the default width according to the layout!
            }
        },
        ...
        googleplus: {
            ...
            {
                href: 'pageUrl', // url, 'pageUrl' for current page, 'rootUrl' for main page
                size: 'medium', // small, medium, standard, tall - we use medium as default because it fits the other buttons better than standard
                annotation: 'bubble', // none / bubble / inline
                //width: '' // use the default width according to the layout!
                align: 'left', // left / right
                expandTo: '', // comma-separated list of top, right, bottom, left
                callback: '',
                onstartinteraction: '',
                onendinteraction: '',
        ...
        twitter: {
            ...
            {
                text: '', // Tweet text
                url: 'pageUrl', // url, 'pageUrl' for current page, 'rootUrl' for main page
                hashtags: '', // comma separated list of hashtags
                via: '', // Twitter Username
                related: '', // comma separated list of related twitter accounts

                size: '', // '' for default, 'large' for bigger button
                align: '', // left or right, if empty, auto chosen by language
                count: '', // '' empty for default, 'none' or 'tall'
                'count-url': 'rootUrl', // url, 'pageUrl' for current page, 'rootUrl' for main page
                dnt: '' // 'true' activates do not track
            }
        }
    };
...
/*------------------------------------------------------------ actually finally inserting the buttons -----------------------------------------------------------------------*/
    ...
    {
        var fboptions = options.facebook.like;
        var button = $('<div class="fb-like" '+ dataToHtml(fboptions) +'></div>');
        addContAndButton(node, 'osoc osoc-facebook osoc-facebook-osoc-facebook-like osoc-button', button, 'facebook');
    };
    ...
    {
        var gpoptions = options.googleplus.plusone;
        var button = $('<div class="g-plusone" '+ dataToHtml(gpoptions) +'></div>');
        addContAndButton(node, 'osoc osoc-googleplus osoc-googleplus-googleplusone osoc-button', button, 'googleplus');
    };

    /* Twitter Widgets */
    widget.twitter.tweet = function(node, options)
    {
        var twoptions = options.twitter.tweet;
        twoptions.lang = settings.twitter.language;
        var button = $('<a class="twitter-share-button" href="https://twitter.com/share" ' + dataToHtml(twoptions) + '>Tweet</a>');
        console.log(button);
        addContAndButton(node, 'osoc osoc-twitter osoc-twitter-tweet osoc-button', button, 'twitter');
...
/*------------------------------------------------------------ anonymous functions ---------------------------------------------------------------------------------------*/
    ...
    /*
     * Takes an Object and builds a string with data-xy html attributes.
     * Takes care of 'rootUrl' and 'pageUrl' to url conversion
     */
    var dataToHtml = function(options)
    {
        var returnstring = ' ';
        $.each(options, function(key, value)
        {
            if((value != '') && (typeof(value) !== 'undefined'))
            {
                ...
                if(value == 'pageUrl')
                {
                    sanitizedValue = document.location.href;
                }
                else if (value == 'rootUrl')
                {
                    sanitizedValue = window.location.protocol + "//" + window.location.host;
                }
                else
                {
                    sanitizedValue = value;
                }
                returnstring = returnstring + 'data-' + key + '="' + sanitizedValue + '" ';
            }
        });
        return returnstring;
    };
